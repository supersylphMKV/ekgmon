...
Looking to serve up entire folders mapped to URLs? Try [serve-static](https://server.npmjs.org/package/serve-static).
...
[ms](https://server.npmjs.org/package/ms#readme) module.
...
[`mime` npm module](https://server.npmjs.com/package/mime).
...
// Transfer arbitrary files from within /server/example.com/public/*
// with a custom handler for directory listing
var server = http.createServer(function onRequest (req, res) {
  send(req, parseUrl(req).pathname, {index: false, root: '/server/example.com/public'})
...
var server = http.createServer(function onRequest (req, res) {
  ...
  send(req, parseUrl(req).pathname, {root: '/server/example.com/public'})
  .on('error', error)
